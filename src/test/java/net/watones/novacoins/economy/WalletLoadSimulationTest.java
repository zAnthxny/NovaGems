...
import net.watones.novacoins.activity.ActivitySignal;
import net.watones.novacoins.activity.RollingActivityProfile;
import net.watones.novacoins.session.SessionAccumulator;
...
class WalletLoadSimulationTest {
  ...
  void fiveHundredCompletedRewardsRecoverExactlyOnceWithoutPlayerThreads(@TempDir Path temp)
    ...
    long novaThreads = Thread.getAllStackTraces().keySet().stream()
        .map(Thread::getName).filter(name -> name.startsWith("NovaCoins-")).count();
  ...
  void boundedSQLiteWriterPreservesEveryBalance(int players, @TempDir Path temp) throws Exception {
    ...
    long novaThreads =
        Thread.getAllStackTraces().keySet().stream()
            .map(Thread::getName)
            .filter(name -> name.startsWith("NovaCoins-"))
            .count();
    assertThat(novaThreads).isLessThanOrEqualTo(3);
    wallets.close();
    storage.close();
