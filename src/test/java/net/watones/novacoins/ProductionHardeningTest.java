...
import net.watones.novacoins.economy.*;
import net.watones.novacoins.session.CompletedReward;
import net.watones.novacoins.session.PendingCompletedRewards;
import net.watones.novacoins.shop.PurchasePolicy;
import net.watones.novacoins.storage.*;
...
class ProductionHardeningTest {
  ...
  void fiveHundredRewardJournalStormUsesOneBoundedWriter() throws Exception {
    ...
    assertThat(Thread.getAllStackTraces().keySet().stream()
        .filter(thread -> thread.getName().equals("NovaCoins-JournalWriter"))).hasSize(1);
  ...
  void concurrentAccountSequencesArePositiveAndUnique() throws Exception {
    ...
    List<Long> sequences = database.history(player, 0, 50).stream()
        .map(CoinTransaction::accountSequence).toList();
  ...
  private static class CountingUnavailableStorage implements StorageProvider {
    ...
    public Optional<CoinTransaction> findTransaction(UUID id) { return Optional.empty(); }
    public List<LeaderboardEntry> leaderboard(int offset, int limit) { return List.of(); }
    public List<CoinTransaction> history(UUID uuid, int offset, int limit) { return List.of(); }
    public List<CoinTransaction> deliveryFailures(int offset, int limit) { return List.of(); }
  ...
  private static final class ToggleStorage implements StorageProvider {
    ...
    public List<CoinTransaction> history(UUID id, int offset, int limit) throws Exception { check(); return delegate.history(id, offset, limit); }
