...
import net.watones.novacoins.activity.ConservativeActivityGuard;
import net.watones.novacoins.command.WataCoinsCommand;
...
import net.watones.novacoins.listener.ActivityListener;
import net.watones.novacoins.listener.PlayerConnectionListener;
import net.watones.novacoins.message.MessageService;
import net.watones.novacoins.placeholder.NovaCoinsExpansion;
import net.watones.novacoins.session.RewardService;
import net.watones.novacoins.session.SessionRegistry;
import net.watones.novacoins.session.SessionService;
import net.watones.novacoins.shop.ShopService;
import net.watones.novacoins.shop.listener.ShopListener;
...
public final class NovaCoinsPlugin extends JavaPlugin {
  ...
  public void onEnable() {
    ...
    Thread.ofPlatform().name("NovaCoins-Bootstrap").start(this::bootstrapOffThread);
  }
  ...
  private void bootstrapOffThread() {
    try {
      ...
      getLogger().log(Level.SEVERE, "NovaCoins no puede iniciar de forma segura", failure);
      DiscordWebhookAlertService currentAlerts = alerts;
  ...
  private void finishEnable(BootstrapContext context) {
    ...
    WataCoinsCommand wataCoins = new WataCoinsCommand(
    ...
    command("watacoins").setExecutor(wataCoins);
    command("watacoins").setTabCompleter(wataCoins);
    ...
    context.wallets.corruptRecoveryRecords().thenAccept(records -> {
      if (!records.isEmpty() && alerts != null) alerts.alert("journal-corrupt",
          ...
          records.size() + " registro(s) en cuarentena. Usa /watacoins admin recovery corrupt.");
    });
    ...
    if (Bukkit.getPluginManager().isPluginEnabled("PlaceholderAPI")) {
      new NovaCoinsExpansion(context.wallets, context.registry, context.config).register();
    ...
    getLogger().info("NovaCoins v" + getPluginMeta().getVersion() + " habilitado");
  }
  ...
  private void checkOperationalAlerts() {
    ...
    if (wallets.journalHealth() != net.watones.novacoins.storage.JournalHealth.HEALTHY) {
