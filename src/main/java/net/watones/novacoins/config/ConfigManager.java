package net.watones.novacoins.config;
...
public final class ConfigManager {
  ...
  public RuntimeConfig parseCandidate() {
    ...
    long coins = config.getLong("rewards.coins-per-interval", 10);
    if (interval <= 0 || interval > 31_536_000) {
      throw new IllegalArgumentException(
          "rewards.interval-seconds debe estar entre 1 y 31536000");
    }
    if (coins <= 0 || coins > 1_000_000_000_000L) {
      throw new IllegalArgumentException(
          "rewards.coins-per-interval debe estar entre 1 y 1000000000000");
    }
  ...
  private RuntimeConfig.StorageSettings parseStorage(FileConfiguration config) {
    ...
    String sqliteFile = config.getString("storage.sqlite.file", "novacoins.db");
    ...
    String database = config.getString("storage.mysql.database", "novacoins");
