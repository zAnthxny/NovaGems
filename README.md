...
- Un único `NovaCoins-JournalWriter` con cola acotada realiza write, fsync y rename fuera del hilo de Paper. Una operación permanece `CAPTURING` hasta confirmar almacenamiento durable; sólo entonces pasa a SQL. El replay es *at least once* y el efecto económico es *exactly once* por `operation_id`.
...
Antes de ejecutar una acción irreversible, NovaCoins confirma `DELIVERY_STARTED`. Si el servidor reinicia antes de confirmar el resultado, pasa a `MANUAL_REVIEW`: no repite el comando ni devuelve saldo automáticamente. Una reward insegura se deshabilita sin afectar las demás. Se recomienda que el receptor acepte `<operation_id>`.

## Instalación y compilación

1. Compila con `./gradlew clean test build`.
2. Elige **una sola** distribución:
   - `build/libs/NovaCoins-1.1.4.jar` (recomendada): JAR slim; Paper resuelve SQLite JDBC, MySQL Connector/J y HikariCP desde Maven durante el arranque.
   - `build/libs/NovaCoins-1.1.4-offline.jar`: JAR autocontenido para hosts sin acceso a Maven durante el arranque.
...
El slim necesita acceso al repositorio Maven configurado por Paper en el primer arranque; si la resolución falla, Paper puede no cargar NovaCoins. Los arranques posteriores reutilizan la caché de libraries. El tamaño pequeño corresponde al artefacto de NovaCoins: las dependencias externalizadas siguen ocupando espacio en la caché del servidor. Para un host sin salida a Internet usa el JAR offline.

## Comandos

- `/watacoins` — abre directamente el menú de canjes.
- `/watacoins balance` — muestra el saldo propio.
- `/watacoins help` — muestra la ayuda en forma de lista.
- `/watacoins admin give|take|set <jugador> <cantidad>` — sólo operadores.
- `/watacoins admin reset <jugador>` — sólo operadores.
- `/watacoins admin reload` — sólo operadores.
- `/watacoins admin status` — estado compacto de economía, journal y webhook.
- `/watacoins admin review [operación]` — consulta y resolución confirmada de `MANUAL_REVIEW`.
- `/watacoins admin recovery [corrupt]` — ejecuta un batch seguro o lista cuarentenas.

Las acciones bajo `/watacoins admin` comprueban directamente el estado OP y no pueden habilitarse mediante permisos administrativos granulares.

## PlaceholderAPI

Todos los placeholders leen caché O(1) y nunca hacen SQL:

- `%novacoins_balance%`, `%novacoins_balance_formatted%`
- `%novacoins_lifetime_earned%`, `%novacoins_lifetime_spent%`
- `%novacoins_session_elapsed%`, `%novacoins_cycle_elapsed%`
- `%novacoins_session_remaining%`, `%novacoins_cycle_remaining%`
- `%novacoins_session_cycles%`, `%novacoins_reward_progress_percent%`
- `%novacoins_account_state%`, `%novacoins_storage_health%`
...
`/watacoins admin reload` primero parsea y valida candidatos completos de `config.yml`, `messages.yml` y `shop.yml`; sólo después intercambia las tres instantáneas. Una reward individual inválida se omite con WARN; un YAML estructuralmente inválido rechaza el candidato. Las sesiones siguen vivas.
